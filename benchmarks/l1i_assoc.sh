...
benchmark="specbzip/l1i_assoc"
for l1i_assoc in 1 2 4 8 16
do
    eval ${gem5}/build/ARM/gem5.opt -d ${benchmark}/${l1i_assoc} ${gem5}/configs/example/se.py ${options} --${testVariable} ${l1i_assoc} -c ${command} -I 100000000 &
done
wait

command="${program_location}/429.mcf/src/specmcf -o \"${program_location}/429.mcf/data/inp.in\""
benchmark="specmcf/l1i_assoc"
for l1i_assoc in 1 2 4 8 16
do
    eval ${gem5}/build/ARM/gem5.opt -d ${benchmark}/${l1i_assoc} ${gem5}/configs/example/se.py ${options} --${testVariable} ${l1i_assoc} -c ${command} -I 100000000 &
done
